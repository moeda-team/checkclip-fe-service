...
import { Megaphone, Users, DollarSign, FileText, Smartphone } from "lucide-react";
import { cn } from "@/lib/utils";
import type { CampaignObjectiveKey, CampaignObjectiveType } from "@/types/campaign";
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 items-stretch">
      {objectives.map((obj) => (
        ...
        >
          ...
          <div className="h-28 shrink-0 bg-gradient-to-br from-purple-600 to-violet-700 flex items-center justify-center">
            {obj.icon}
          </div>
          {/* Content — grows to fill remaining space */}
          <div className="flex flex-col flex-1 p-4 gap-1.5">
            <p className="text-sm font-semibold text-gray-900">{obj.label}</p>
            <span className={cn("inline-block w-fit text-xs px-2 py-0.5 rounded-full font-medium", obj.badgeColor)}>
              {obj.badge}
            </span>
            <p className="text-xs text-gray-500 leading-relaxed mt-auto pt-1">{obj.description}</p>
          </div>
        </button>
      ))}
    </div>
