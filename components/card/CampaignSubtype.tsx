...
export const campaignSubtypeOptions: Record<CampaignObjectiveKey, SubtypeOption[]> = {
  awareness: [
    {
...
export function CampaignSubtype({ objective, value, onChange }: Props) {
  ...
  const selectedDescription = options.find((s) => s.label === value)?.description;
    ...
    <div className="space-y-1.5">
      <Label className="text-sm font-medium text-gray-700 flex items-center gap-1">
        ...
        <Tooltip>
          ...
          <TooltipContent side="top" className="max-w-xs text-xs bg-gray-900 text-white">
      ...
      <Select value={value} onValueChange={onChange}>
        <SelectTrigger className="h-10 border-gray-200 text-sm focus:ring-purple-500">
        ...
        <SelectContent>
          {options.map((sub) => (
            <SelectItem key={sub.label} value={sub.label} className="items-start py-2">
      ...
      {selectedDescription && (
        <p className="text-xs text-gray-500 leading-relaxed">{selectedDescription}</p>
      )}
    </div>
  );
}
