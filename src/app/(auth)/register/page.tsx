  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className={cn(
        ...
        error ? "border-red-500" : "border-slate-200 focus-within:border-lavender focus-within:ring-4 focus-within:ring-lavender/10"
      )}>
        <Icon className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" size={20} />
        <input
        ...
        <div className="flex flex-col flex-1 justify-between">
          ...
          <div className="flex-1 flex flex-col justify-center">
            ...
            <p className="text-base text-white/80 m-0 mb-8 leading-relaxed">
              Empower your enterprise with seamless tax calculators, real-time clock logs,
              local BPJS sync, and active KPI targets.
            </p>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="flex flex-col gap-2">
                ...
                >
                  <SelectTrigger className="w-full border-2 border-slate-200 rounded-[14px] h-[48px] pl-[52px] relative">
                    <Briefcase className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" size={20} />
                ...
                {errors.job_title && <span className="text-[13px] text-red-500 font-medium">{errors.job_title}</span>}
              </div>

              <div className="flex flex-col gap-2">
                <Label className="text-sm font-semibold text-slate-700">Department Unit</Label>
                <Select
                ...
                {errors.department_unit && <span className="text-[13px] text-red-500 font-medium">{errors.department_unit}</span>}
              </div>

              <div className="flex flex-col gap-2">
                <Label className="text-sm font-semibold text-slate-700">System Auth Role</Label>
                <Select
                  value={formData.role}
                  onValueChange={handleInputChange("role")}
                >
                  <SelectTrigger className="w-full border-2 border-slate-200 rounded-[14px] h-[48px] pl-[52px] relative">
                    <Layers className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" size={20} />
                ...
                {errors.role && <span className="text-[13px] text-red-500 font-medium">{errors.role}</span>}
              </div>
            ...
            <div className="flex flex-col gap-6">
              <div className="flex flex-col gap-2">
                ...
                <div className={cn(
                  "relative flex items-start border-2 rounded-[14px] bg-white transition-all",
                  errors.address ? "border-red-500" : "border-slate-200 focus-within:border-lavender focus-within:ring-4 focus-within:ring-lavender/10"
                )}>
                  <MapPin className="absolute left-4 top-[14px] text-slate-400 pointer-events-none" size={20} />
                  <textarea
                    placeholder="Enter your residential address"
                    value={formData.address}
                    onChange={(e) => handleInputChange("address")(e.target.value)}
                    className="w-full py-[14px] px-4 pl-[52px] border-none rounded-[14px] bg-transparent font-sans text-[15px] text-midnight outline-none placeholder:text-slate-400 resize-y min-h-[80px] leading-relaxed"
                    rows={3}
                    autoCapitalize="off"
                    autoCorrect="off"
                  />
                </div>
                {errors.address && <span className="text-[13px] text-red-500 font-medium">{errors.address}</span>}
              </div>
            ...
            >
              {isLoading ? "Creating Account..." : "Create Employee Record →"}
            </Button>

            <div className="text-center mt-6">
              <p className="text-sm text-slate-500 m-0">
                Already registered with corporate credential?{" "}
                <Link href="/login" className="text-lavender no-underline font-semibold ml-1 hover:underline">
