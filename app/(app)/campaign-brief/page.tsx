...
import {
  ...
  Settings2, DollarSign, Users, Megaphone,
...
import { CampaignBrief, CampaignBriefFilter, CampaignBriefStatus } from "./types";
import { AdsIcon, AllSections, BriefSkeleton } from './function';
import { useGetStrategyBriefs, useGetStrategyBrief } from "./hooks/useCampaignBrief";
    ...
    <div className="flex h-[calc(100vh-56px)] overflow-hidden">
      ...
      <div className="w-56 shrink-0 border-r border-gray-200 flex flex-col bg-white">
        <div className="p-3 border-b border-gray-100">
          <Button onClick={() => router.push("/campaign-brief/create")}
        ...
        <div className="p-3 border-b border-gray-100">
          <div className="relative">
            ...
            <Input placeholder="Type here" value={searchInput}
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? <BriefSkeleton /> : isListError ? (
            <div className="p-4 text-xs text-red-500 text-center space-y-2">
              <p>Failed to load briefs</p>
              <button onClick={() => refetchList()} className="text-purple-600 underline">Retry</button>
            </div>
          ) : briefs.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full py-12 px-4 text-center gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-gray-600">{debouncedSearch ? "No results found" : "No briefs yet"}</p>
            ...
            briefs.map((brief) => {
              ...
              const status = statusConfig[brief.status] ?? { label: brief.status, color: "text-gray-400" };
              return (
                <button key={brief.id} type="button" onClick={() => setSelectedBriefId(brief.id)}
                  className={cn("w-full text-left px-3 py-3 border-b border-gray-100 flex items-start gap-2.5 transition-colors",
                    isSelected ? "bg-purple-50 border-l-2 border-l-purple-600" : "hover:bg-gray-50 border-l-2 border-l-transparent")}>
                  <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center shrink-0 mt-0.5", isSelected ? "bg-purple-600" : "bg-gray-100")}>
                    <FileText className={cn("w-4 h-4", isSelected ? "text-white" : "text-gray-500")} />
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className={cn("text-xs font-semibold leading-snug line-clamp-2", isSelected ? "text-purple-700" : "text-gray-900")}>{brief.title}</p>
                    <p className={cn("text-xs mt-0.5 flex items-center gap-1", status.color)}>
                      <span className="w-1.5 h-1.5 rounded-full bg-current inline-block" />{status.label}
                    </p>
                  </div>
                  <ChevronRight className={cn("w-3.5 h-3.5 shrink-0 mt-1", isSelected ? "text-purple-600" : "text-gray-400")} />
                </button>
              );
            })
          )}
        ...
        {totalPages > 1 && (
          <div className="border-t border-gray-100 px-3 py-2 flex items-center justify-between">
            <button disabled={offset === 0} onClick={() => setOffset((o) => Math.max(0, o - PAGE_LIMIT))}
              className="p-1 rounded hover:bg-gray-100 disabled:opacity-30">
              <PrevIcon className="w-3.5 h-3.5 text-gray-600" />
            </button>
            <span className="text-xs text-gray-500">{currentPage} / {totalPages}</span>
            <button disabled={offset + PAGE_LIMIT >= total} onClick={() => setOffset((o) => o + PAGE_LIMIT)}
              className="p-1 rounded hover:bg-gray-100 disabled:opacity-30">
      ...
      {detailLoading ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-3 text-sm">
          ...
          <button onClick={() => refetchDetail()} className="text-purple-600 underline text-xs">Retry</button>
        </div>
      ) : selectedBrief ? (
        <div className="flex-1 flex flex-col overflow-hidden">
          {/* Header */}
          <div className="px-6 py-4 border-b border-gray-200 bg-white flex items-center justify-between shrink-0">
            <div>
              <h1 className="text-base font-semibold text-gray-900">{selectedBrief.title}</h1>
              <div className="flex items-center gap-1.5 mt-0.5">
                <AdsIcon type={selectedBrief.type_ads} />
                <span className="text-xs text-gray-500 capitalize">{selectedBrief.type_ads} Ads</span>
              </div>
            </div>
          ...
          <div className="flex flex-1 overflow-hidden">
            {/* Section nav */}
            <div className={cn("border-r border-gray-200 bg-white flex flex-col shrink-0 transition-all duration-200", sectionPanelOpen ? "w-52" : "w-10")}>
                    ...
                    return (
                      <button key={sec.key} type="button" onClick={() => scrollToSection(sec.key)}
                        className={cn("w-full flex items-center gap-2.5 px-4 py-2.5 text-sm transition-colors",
                          isActive ? "text-purple-700 font-semibold bg-purple-50" : "text-gray-600 hover:bg-gray-50")}>
                        <Icon className={cn("w-4 h-4 shrink-0", isActive && "text-purple-600")} />
                        {sec.label}
                      </button>
                    );
                  })}
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-3 text-center px-6">
          ...
          <div>
            <p className="text-sm font-medium text-gray-500">{debouncedSearch ? "No matching briefs" : "No briefs available"}</p>
            <p className="text-xs text-gray-400 mt-0.5">{debouncedSearch ? "Try a different search term" : "Select a brief from the list or create a new one"}</p>
          </div>
        </div>
      )}
    </div>
